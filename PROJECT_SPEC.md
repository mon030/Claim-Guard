...
The user supplied `guardrail.config.ts`, `guardrail.ts`, `guardrail.test.ts`, and `claims.json` from `E:/Downloads`. The three TypeScript files belong directly in `lib/`; claims belong in `data/claims.json` and must not be re-derived. Preserve the supplied evidence/config types and all decision thresholds. Thresholds remain the team's placeholders until they review real-photo validation. Two explicitly authorized exceptions to verbatim copying are recorded in `docs/PROMPTS.md`: add `export` to `hammingDistanceHex`, and require zero non-stock full matches before writing the stock-only note. The original nine supplied tests remain unchanged.
...
Local .env settings were imported from ignored env_setup.md; optional LLM settings remain incomplete and disabled. The user authorized a full validation on 2026-09-25: using cached Vision results where available and live calls otherwise, all 36 reference photos had usable evidence on the second pass, with 31 distinct byte hashes and five byte-identical pairs. Actual results are in `docs/vision-validation.txt` and `.json`; no thresholds were changed. All 18 claims were run through the local lookup/decision APIs. An intended dry-run verification in an earlier phase seeded 18 claims/36 photos because npm consumed the flag; disclosed to the user, fixed by honoring npm_config_dry_run, and verified with explicit aliases. No Vision calibration results or photo mappings were invented. Assignment deliverables also include a guardrail justification and requirements/wireframe document.
